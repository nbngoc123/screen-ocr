...
def main():
    ...
    parser.add_argument("--train-dir", type=str, default=None, help="Thư mục dữ liệu train")
    parser.add_argument("--val-dir", type=str, default=None, help="Thư mục dữ liệu val")
    ...
    epochs = args.epochs if args.epochs is not None else config["training"]["epochs_phase1"] + config["training"]["epochs_phase2"] + config["training"]["epochs_phase3"]
    ...
    log_file_path = Path(config["paths"].get("reports", "reports")) / "training_log.csv"
    ...
    for epoch in range(1, epochs + 1):
        ...
        with open(log_file_path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([epoch, f"{train_loss:.4f}", f"{val_loss:.4f}", f"{optimizer.param_groups[0]['lr']:.6f}", f"{elapsed:.1f}"])
